...
DEFAULT_REVIEW_LABELS = "data/labels/trajectory_reviews.csv"
MIN_MODEL_TRAJECTORY_TRANSITIONS = 3
...
def _trusted_plausible_trajectory_ids(
    ...
    if not path.exists():
        return frozenset()
    try:
        from modeling.labels.plausible_labels import read_plausible_label_table

        labels = read_plausible_label_table(path)
        labels = labels.loc[
        ...
        if labels.empty:
            return frozenset()
        feature_ids = pd.read_parquet(
            Path(feature_root) / feature_set / dataset / "trajectories.parquet",
            columns=["trajectory_id"],
        )
        ids = labels["trajectory_id"].astype(np.int64)
        ids = ids.loc[ids.isin(feature_ids["trajectory_id"].astype(np.int64))]
        return frozenset(ids.astype(np.int64).tolist())
...
@lru_cache(maxsize=128)
def _reviewed_benchmark_trajectory_ids(
    feature_root,
    processed_root,
    feature_set,
    dataset,
    ...
    if not path.exists():
        return frozenset()
    try:
        from modeling.labels.review_labels import read_review_label_table

        labels = read_review_label_table(path)
        labels = labels.loc[
            labels["label"].isin({"plausible", "unplausible"})
            & labels["trajectory_id"].astype(str).str.strip().ne("")
            & labels["dataset"].astype(str).eq(str(dataset))
        ]
        if labels.empty:
...
def validate_training_trajectories(
    ...
    _require_no_overlap(
        dataset,
        selected_ids,
        _reviewed_benchmark_trajectory_ids(
            str(feature_root),
            DEFAULT_PROCESSED_ROOT if processed_root is None else str(processed_root),
            feature_set,
            dataset,
...
def filter_model_trajectory_lengths(
    trajectory_table,
    dataset,
    min_transitions=MIN_MODEL_TRAJECTORY_TRANSITIONS,
):
    if "n_transitions" not in trajectory_table.columns:
        raise ValueError(
            f"{dataset}: trajectory table is missing n_transitions; "
            "cannot enforce the minimum trajectory length"
        )
    return trajectory_table.loc[
        trajectory_table["n_transitions"].astype(np.int64) >= min_transitions
    ].copy()


def eligible_training_count(feature_root, feature_set, dataset):
    table = pd.read_parquet(
        Path(feature_root) / feature_set / dataset / "trajectories.parquet",
        columns=["use_for_training", "n_transitions"],
    )
    table = filter_model_trajectory_lengths(table, dataset)
    return int(table["use_for_training"].sum())
...
class ParquetTrajectoryDataset(Dataset):
    def __init__(
        ...
        trajectory_table = filter_model_trajectory_lengths(trajectory_table, dataset)
        if training_only:
...
def load_combined_datasets(
    ...
    if training_only and sampling_strategy == "balanced" and len(datasets) > 1:
        ...
        for dataset in datasets:
            counts.append(eligible_training_count(feature_root, feature_set, dataset))
