...
def trajectory_physical_metrics(processed_root, dataset, trajectory_ids):
    ...
    return (
        ...
        .agg(
            ...
            avg_elapsed_time=("elapsed_time_s", "mean"),
        )
        .reset_index()
    )
...
def population_values(
    ...
    if metric in {"max_speed", "total_distance", "total_elapsed_time", "avg_elapsed_time"}:
...
def build_model_population_stats(
    ...
    feature_dir = Path(feature_root) / feature_set / dataset
    if not feature_dir.exists():
        feature_dir = Path(feature_root) / "motion_v2" / dataset
    trajectories = pd.read_parquet(
        feature_dir / "trajectories.parquet",
        columns=[
            "trajectory_id",
            "n_transitions",
            "use_for_calibration",
